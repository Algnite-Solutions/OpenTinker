...
class DynamicGameDatasetVL(DynamicGameDataset):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            tokenizer=tokenizer or processor.tokenizer,  # Use processor's tokenizer if no separate tokenizer
            config=config,
            processor=processor,
        ...
        logger.info(f"DynamicGameDatasetVL initialized with processor: {processor is not None}")
    ...
    def __getitem__(self, item: int) -> Dict[str, Any]:
        ...
        if (self.processor is not None and 
            ...
            try:
                ...
                import logging
                logging.warning("Could not import verl.models.transformers.qwen2_vl.get_rope_index, "
        ...
        for key, val in extra_model_inputs.items():
            if val is not None:
                ...
                row_dict[key] = val[0] if len(val.shape) > 0 and val.shape[0] == 1 else val
