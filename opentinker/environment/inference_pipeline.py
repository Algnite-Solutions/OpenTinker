...
class RemoteEnvironmentClient:
    ...
    def __init__(
        self, env_endpoint: str, job_id: str = "default", timeout: float = 60.0
    ):
...
class InferencePipeline:
    ...
    def __init__(
        ...
        print(
            f"✓ Inference pipeline initialized ({self.mode} mode, job_id={self.job_id})"
        )
    ...
    async def run_single_inference(
        ...
        while not done:
            ...
            if total_response_tokens >= max_tokens:
                print(
                    f"⚠ Total response tokens ({total_response_tokens}) reached limit ({max_tokens}), ending"
                )
                break
            ...
            if prompt_tokens > self.max_context_length:
                print(
                    f"⚠ Context length ({prompt_tokens}) exceeds limit ({self.max_context_length}), ending game"
                )
                break
...
def load_samples(
    ...
    **game_kwargs,
    ...
    if data_path is not None:
        ...
        num_samples = (
            len(generator) if max_samples is None else min(max_samples, len(generator))
        )
        ...
        if max_samples is None:
            raise ValueError(
                "max_samples is required when data_path is None (dynamic generation)"
            )
        ...
        print(
            f"Generated {len(samples)} samples dynamically using {game_class.__name__}"
        )
