...
class GenericEnvironment(BaseEnvironment):
    ...
    def _setup_dataloader(self):
        ...
        data_config = OmegaConf.create({
            ...
            "val_files": [self.config.val_data_path] if getattr(self.config, 'val_data_path', None) else [],
        ...
        print(f"Sample raw_prompt: {str(first_batch.get('raw_prompt', ['N/A'])[0])[:100]}...")
    ...
    def _setup_interaction_config(self):
        ...
        self._interaction_config_content = yaml.dump(config_dict, default_flow_style=False)
            ...
            config["actor_rollout_ref"] = {
                "rollout": {
                    "multi_turn": {
                        ...
                        "interaction_config_content": getattr(self, '_interaction_config_content', None),
                    },
                    "agent": {
    ...
    def cleanup(self):
        ...
        if self._interaction_config_path and os.path.exists(self._interaction_config_path):
            os.remove(self._interaction_config_path)
            print(f"Removed temporary interaction config: {self._interaction_config_path}")
